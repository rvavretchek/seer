...
**#1 — Claude Code environment.** Run end to end on a real request (not a rehearsal of the worked example above): *"Faça uma análise do crescimento vegetativo entre 1980 e 2025 da população mundial na faixa etária de 0 a 15 anos a partir dos relatórios da PNUD e OCDE e no Brasil o PNAD contínuo."* Steps 1-5 all executed for real: Step 1 caught that the request's named sources (UNDP, PNAD Contínua alone) don't actually fit the ask before any dispatch happened — UNDP sources its own demographic figures secondhand from UN DESA rather than publishing primary data, and PNAD Contínua only exists since 2012, so a 1980-2025 Brazil series necessarily splices in Censo Demográfico and legacy PNAD data. Step 4 dispatched two independent research subagents in parallel (global trend via UN WPP 2024; Brazil trend via IBGE) with real, cited findings, not fabricated figures. Step 5 synthesized both into one answer, leading with the source-fit caveat rather than burying it. The dispatch mechanism itself worked as designed — in the environment this ran in, the tool available for spawning subagents was named `Agent` rather than `Task`; the dispatch *logic* (spawn a subagent per plan item, run independent ones in parallel, synthesize) held regardless of the tool's name for that surface.

**#2 — Real Claude Cowork app (v1.0.0-rc.1), resolving the open question below.** Installed via `Add marketplace: rvavretchek/seer`, then given: *"Analise como a queda da taxa de natalidade no Nordeste brasileiro pode afetar o planejamento educacional regional nos próximos 10 anos."* Full test script and this run's context: `tests/manual/cowork-rc1-test-script.md`. Confirmed inside Cowork itself, not a proxy environment:

- Discovery and planning worked: the orchestrator scanned the installed skill roster and built a plan without a hardcoded table.
- **Parallel dispatch really happened** — `human-geography` and `economic-geography` ran as independent subagents at the same time, each producing its own interpretation (fertility vs. migration effects; municipal fiscal dependency on FUNDEB) without cross-contaminating the other's reasoning.
- **The "apply directly, skip the subagent" case emerged on its own** — `educational-policy` was applied straight in the synthesis step rather than dispatched, because its inputs were already in hand. This wasn't explicitly scripted in Step 4 above; it's a sensible reading of "dispatch what needs its own reasoning pass," not a deviation to correct.
- `regional-analysis` was correctly *not* invoked, with the model's own stated reason matching this skill's actual scope (it's for comparing multiple regions; this request was one region, two lenses) — evidence the plan-building step is reading `description`/scope, not just keyword-matching skill names.
- Step 5's synthesis discipline held under real pressure: the answer explicitly flagged the tension between an earlier migration-driven reading and newer birth-decline data, rather than quietly picking one — the exact behavior Known Limitations below and Constitution Principle 6 call for.

## Known Limitations

- **The dispatch tool's name varies by surface.** This skill's Procedure names `Task` (the Claude Code tool); both proof-of-concept runs above executed the same dispatch *logic* via whatever subagent mechanism each surface actually exposed (`Agent` in one harness, Cowork's own internal mechanism in the other — never observed directly, only its effects). Treat "spawn a subagent to run this skill's procedure" as the portable instruction; the exact tool name is an adapter-layer detail, not something to hardcode expectations around.
