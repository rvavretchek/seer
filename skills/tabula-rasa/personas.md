...
Maps a discovered skill's `domain` to the voice Tabula Rasa gives it in the room. A matched skill with no entry here still joins the room -- it speaks under its own skill title as a neutral voice until this file grows to cover it (see `SKILL.md`, Known Limitations).
