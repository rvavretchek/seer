...
6. **When no pre-packaged layer covers the question, query the source API directly over HTTP — INPE's TerraBrasilis GeoServer is the concrete Brazil case.** GeoServer exposes data via OGC WFS: build a `GetFeature` request naming the target layer with `typeName` and filtering with CQL (Common Query Language, WFS's SQL-like filter syntax). For example, DETER-Amazônia deforestation alerts within a date range:

   ```bash
   curl "https://terrabrasilis.dpi.inpe.br/geoserver/deter-amz/wfs?service=WFS&version=2.0.0&request=GetFeature&typeName=deter_public&CQL_FILTER=date BETWEEN '2024-01-01' AND '2024-06-30'&outputFormat=application/json"
   ```

   This returns GeoJSON for every alert polygon between January and June 2024 — `date BETWEEN 'X' AND 'Y'` is the CQL date-range pattern; narrow further by combining filters with `AND` (e.g. `CQL_FILTER=date BETWEEN '...' AND '...' AND municipality='<name>'`). `typeName` picks the specific layer on the GeoServer instance (`deter_public` here covers the Amazon biome; other biomes and programs live under different `typeName`/workspace values on the same server — run a `GetCapabilities` request first if the target layer name isn't already known). Fire-hotspot (queimadas) data, by contrast, isn't a query endpoint at all: `https://dataserver-coids.inpe.br/queimadas/queimadas/focos/csv/{mensal,anual}/Brasil/` serves pre-built CSV/KML files partitioned by month or year — list the directory to find the current file name, `curl -O` it, then filter locally rather than expecting a CQL-style filter on the download itself. Treat the WFS/CQL pattern itself as reusable for any GeoServer-backed source, not INPE-specific; see `geography/physical-geography` for what PRODES vs. DETER actually measure and when each program is the right one to reach for.
...
- **Direct HTTP calls to a source API like INPE's GeoServer hit a live public government endpoint, not a versioned package.** Paths, `typeName` layer names, and CQL support can change without a deprecation notice; an empty result set or a sudden error from a query that used to work is a signal to re-run `GetCapabilities`, not necessarily proof the CQL syntax is wrong. WFS/CQL has a real learning curve past the worked example above — test an unfamiliar `CQL_FILTER` against a narrow date range before running it at scale. And GeoServer responses declare their own CRS (frequently EPSG:4326) — check it explicitly against whatever CRS the rest of the workflow uses instead of assuming a match, the same discipline the CRS-mismatch bullet above already asks for.
